...
vignette: >
  %\VignetteIndexEntry{MajorTrackR}
...
V(allnets[[2]])$group[V(g1)$group==2][1:45]=3
...
for(i in 1:length(allnets)){
  ...
  plot(currnet,layout=l1,vertex.label=V(currnet)$com,vertex.label.color="white",vertex.label.cex=0.5,
       vertex.color=nodecols,vertex.size=10,
       vertex.shape="fcircle",vertex.frame.color="black",vertex.frame.width=0.5,
       edge.width=E(currnet)$weight/2,edge.color=edgecols)
}
...
Here, the bright pink and light green communities never emerge. The longer history parameter leads to MajorTrack recognising these as being part of the dark blue and yellow communities, as  they merge back into these communities in the subsequent timestep. The dark red community in timestep 4 remains however, as these individuals then merge with the light blue community, rather than returning to the dark blue community.
...
for(i in 1:3){
  ...
  plot(currnet,layout=l2[[i]],vertex.label=V(currnet)$DC,vertex.label.color="white",
       vertex.label.cex=0.5,vertex.color=nodecols,vertex.size=10,
       vertex.shape="fcircle",vertex.frame.color="black",vertex.frame.width=0.5,
       edge.width=E(currnet)$weight/2,edge.color=edgecols)
}
...
```{r visualise matrix,echo = F,out.width="100%",dpi=300,fig.height=2}
par(mar=c(3,2,1,1))
...
Finally you can subset which timesteps get plotted using the rstart and rstop arguments.
...
Here we show off a few particular cases and how the algorithm deals with them. First we generate some similar data. Note that here two equally sized groups merge in timestep 2.

```{r generate networks borderline}
#Generate a network of 200 nodes with all edges
g1=erdos.renyi.game(n=200,p.or.m=1)
#Assign nodes to groups
V(g1)$name=1:200
...
#repeat the network
allnets3=rep(list(g1),6)
#move some individuals between groups


V(allnets3[[2]])$group[V(g1)$group==2]=3

V(allnets3[[3]])$group[V(g1)$group==2][1:20]=3
V(allnets3[[3]])$group[V(g1)$group==3][1:20]=2
V(allnets3[[3]])$group[V(g1)$group==3][21:45]=5
V(allnets3[[3]])$group[V(g1)$group==4][1:10]=1
V(allnets3[[3]])$group[V(g1)$group==4][41:50]=2

V(allnets3[[4]])$group[V(g1)$group==1][1:10]=4
V(allnets3[[4]])$group[V(g1)$group==3][1:20]=2
V(allnets3[[4]])$group[V(g1)$group==3][21:50]=5
V(allnets3[[4]])$group[V(g1)$group==4][1:30]=1
V(allnets3[[4]])$group[V(g1)$group==4][31:50]=2

V(allnets3[[5]])$group[V(g1)$group==1][1:10]=4
V(allnets3[[5]])$group[V(g1)$group==1][40:50]=5
V(allnets3[[5]])$group[V(g1)$group==3][1:20]=2
V(allnets3[[5]])$group[V(g1)$group==3][21:50]=5

#V(allnets[[6]])$group[V(g1)$group==3][1:20]=2
V(allnets3[[6]])$group[V(g1)$group==3][30:50]=1


#alter edge weights depending on if nodes they are in the same group, delete 0 edges
for(i in 1:length(allnets3)){
  currdatalist=as_data_frame(allnets3[[i]],"both")
  currdata=as_long_data_frame(allnets3[[i]])
  currdata$same_group=currdata$from_group==currdata$to_group
  currdata$group=0
  currdata$group[currdata$same_group]=currdata$from_group[currdata$same_group]
  currdata$weight[currdata$same_group]=sample(c(0.9,0.7),sum(currdata$same_group),replace=T,prob=c(0.5,0.5))
  ...
  currdata$timestep=i
  currdatalist$vertices$timestep=i
  allnets3[[i]]=graph_from_data_frame(currdata,directed=F,currdatalist$vertices)
}
...
```{r Detect communities borderline}
#detect each networks communities
coms3=lapply(allnets3,function(x){
	cluster_louvain(x)
})


#apply community membership as node attribute
allnets3=lapply(1:length(allnets3),function(x){
	V(allnets3[[x]])$com=coms3[[x]]$membership
	allnets3[[x]]
})

#Do MajorTrack
track3 <- do_track(allnets3, coms3, history=2)#run MajorTrack
dcmembership3=get_dc_membership(track3)
allnets3 = add_dc_membership(allnets3,dcmembership3)


```


Here are the resulting networks with the dynamic communities.

```{r borderline plotting3,fig.cap="All networks with dynamic communities",dpi=600,echo = F,out.width="100%"}

l3=layout_with_graphopt(allnets3[[1]],niter=1500,charge=0.02,mass=30,spring.length = 0, max.sa.movement	= 5)
l4=list()
l4[[1]]=l3
for(i in 2:length(allnets3)){
  currnet=allnets3[[i]]
  l4[[i]]=layout_with_graphopt(currnet,start=l4[[i-1]],niter=1500,charge=0.02,mass=30,spring.length = 0, max.sa.movement	= 5)
}
...
for(i in 1:length(allnets3)){
  ...
  edgecols=rgb(colorRamp(c("grey","black"))(E(currnet)$weight),maxColorValue=256)
  for(j in unique(E(currnet)$DC[E(currnet)$DC!=-1])){
    currcol=cols[j+1]
    edgecols[E(currnet)$DC==j]=rgb(colorRamp(c(lighten(currcol,0.9),currcol))(E(currnet)$weight[E(currnet)$DC==j]),maxColorValue=256)
  }  
  nodecols = cols[V(currnet)$DC+1]
  ...
  plot(currnet,layout=l3,vertex.label=V(currnet)$DC,vertex.label.color="white",
       vertex.label.cex=0.5,
       vertex.shape="fcircle",vertex.frame.color="black",vertex.frame.width=0.5,
       vertex.color=nodecols,vertex.size=10,
       edge.width=E(currnet)$weight/2,edge.color=edgecols)
...
for(i in 1:length(allnets3)){
  ...
  plot(currnet,layout=l4[[i]],vertex.label=V(currnet)$DC,vertex.label.color="white",
       vertex.label.cex=0.5,
       vertex.shape="fcircle",vertex.frame.color="black",vertex.frame.width=0.5,
       vertex.color=nodecols,vertex.size=10,
       edge.width=E(currnet)$weight/2,edge.color=edgecols)
...
Note that rather than one community remaining during the merging of the yellow and red communities in timestep 2 as the purple community in timestep 2 when the yellow and red communities merge. In this case, this is because  both merging communities were exactly the same size. The algorithm is therefore unable to decide which community merges with which and assigns them a new community
