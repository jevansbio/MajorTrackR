...
knitr::opts_knit$set(root.dir = "C:\\Users\\Julian\\Dropbox\\MOUSEDOC\\MoveGroup")
library(igraph)
library(reticulate)
library(plot.matrix)
library
...
Firstly we need to load the appropriate packages. We'll use igraph to build our networks and reticulate to run python code through R.

```{r load mypython, include = FALSE}
source("MTRfuncs.R",local = knitr::knit_global())
#use_python("C:\\WPy64-3760\\python-3.7.6.amd64")#point to python
use_python("C:\\WPy-3710\\python-3.7.1.amd64")
library(ggthemes)
```

```{r load python,eval=FALSE}
library(igraph)
library(reticulate)
use_python("\\your\\path\\to\\python")#point to python
source("MTRfuncs.R")#load the R functions
```
...
```{r plotting0,fig.cap="All networks",dpi=600,echo = F}
...
for(i in 1:length(allnets)){
  ...
  plot(currnet,layout=l1,vertex.label="",vertex.color=nodecols,vertex.size=10,
       edge.width=E(currnet)$weight,edge.color=edgecols)
}
```

```{r plotting1,fig.cap="All networks",dpi=600,echo = F}
...
```{r plotting2,fig.cap=c("All networks with communities, consistent layout","All networks with communities, per-timestep layout"),dpi=600,echo = F}
...
for(i in 1:length(allnets)){
  ...
  plot(currnet,layout=l2[[i]],vertex.label=V(currnet)$com,vertex.label.color="white",vertex.label.cex=0.75,
       vertex.color=nodecols,vertex.size=10,
       edge.width=E(currnet)$weight,edge.color=edgecols)
}
```
...
Lets feed our list of networks and detected communities into MajorTrack. There is a little prep work we need to do first. Firstly we need to convert our node IDs and community IDs into python style

```{r prepare for MajorTrack}
...
```
After this prep is complete, these variables can be fed into the main MajorTrack function. We also specify the history, start and stop parameters. We'll investigate history some more in a moment, but start and stop are fairly straightforward. If no stop is provided, the algorithm will just run to the end of the timesteps. 
...
```{r plotting3,fig.cap="All networks with dynamic communities",dpi=600,echo = F}
...
for(i in 1:length(allnets)){
  ...
  plot(currnet,layout=l1,vertex.label=V(currnet)$DC,vertex.label.color="white",vertex.label.cex=0.75,
       vertex.color=nodecols,vertex.size=10,
       edge.width=E(currnet)$weight,edge.color=edgecols)
}
...
for(i in 1:length(allnets)){
  ...
  plot(currnet,layout=l2[[i]],vertex.label=V(currnet)$DC,vertex.label.color="white",vertex.label.cex=0.75,
       vertex.color=nodecols,vertex.size=10,
       edge.width=E(currnet)$weight,edge.color=edgecols)
}
...
You might also notice that the merging and  splitting of communities results in completely new dynamic communities being "created", such as the purple community in timestep 2 when the yellow and red communities merge. In this case, this is because  both merging communities were exactly the same size. The algorithm is therefore unable to decide which community merges with which and assigns them a new community
...
A couple of communities exist only for a single timestep (bright pink in timestep 3, light green in timestep 4, dark red in timestep 5). The emergence of these dynamic communities is dependant on the history parameter. Take a look at the same analysis run with a longer history parameter.

```{r different history par}
track2 <- do_track(allpycoms, allpyids, history=6)#run MajorTrack
dcmembership2=get_dc_membership(track2)
allnets2 = add_dc_membership(allnets,dcmembership2)

```

```{r plotting4,fig.cap=c("All networks with dynamic communities with longer history parameters, fixed layout","All networks with dynamic communities with longer history parameters, per-timstep layout"),dpi=600,echo = F}
par(mfrow=c(2,3),mar=c(0,0,0,0))
...
comorigins = move_events_df(track,dcmembership)
head(comorigins, 9)#just the first two timesteps

```

```{r plotting7,fig.cap=c("All networks with dynamic communities showing identifiers, fixed layout","All networks with dynamic communities with longer history parameters, per-timstep layout"),dpi=600,echo = F}
...
for(i in 1:3){
  ...
  plot(currnet,layout=l2[[i]],vertex.label=V(currnet)$DC,vertex.label.color="white",vertex.label.cex=0.75,vertex.color=nodecols,vertex.size=10,
       edge.width=E(currnet)$weight,edge.color=edgecols)
}
...
comorigins2 = move_events_df(track2,dcmembership2)
head(comorigins2, 9)#just the first two timesteps

```

```{r plotting6,fig.cap=c("All networks with dynamic communities showing identifiers, fixed layout","All networks with dynamic communities with longer history parameters, per-timstep layout"),dpi=600,echo = F}
par(mfrow=c(1,3),mar=c(0,0,0,0))
cols=c(gdocs_pal()(10),"yellow","brickred")
...
indmembership = ind_membership_df(dcmembership)
```
This function produces a list of two data frames, each showing individual membership in a different way. The first has timestep, individual ID and DC membership as 3 columns:
...
The second object in the ind_membership_df list is a matrix with a row for each individual and a column for each timestep. This is easily readable way of looking at how an individual's membership changes over time.
...
We could even visualise it:

```{r visualise matrix}

plot(indmembership$memdf2,xlab="Timestep",ylab="Individual",breaks=seq(-0.5,9.5,1),col=c(gdocs_pal()(10)),border=NA,main="",fmt.key="%s",key=NULL, axis.row=NULL)

```

However there is definitely a better way of visualisign this change over time.
...
The R function for the MT alluvial plots is a wrapper for the python plotting functions. It will run some python code, export an image then bring it back into R. By default the python file will then be removed.

This is the simplest type of alluvial plot:

```{r alluvial1,dpi=600,fig.width=8,fig.height=2}



  get_alluvialplot(track,dcmembership,cols)
  

```
Here I provided the MajorTrack object, dynamic community membership and the vector of colours I have been using throughout. At each timestep, the alluvial plot shows the size of the community in that timestep and the flows between them.
...
```{r alluvial2,dpi=600,fig.width=8,fig.height=2}
  #Highlight all moves in blue, remain colour is still in grey
  get_alluvialplot(track,dcmembership,cols,fluxmovecol = "skyblue",fluxremaincol = "grey")
...
```{r alluvial3,dpi=600,fig.width=8,fig.height=2}
  
  get_alluvialplot(track,dcmembership,cols,fluxsinglecol = F, fluxsinglecolremain = F)
...
```{r alluvial4,dpi=600,fig.width=8,fig.height=2}
  
  get_alluvialplot(track,dcmembership,cols,fluxsinglecol = F,fluxbysource = F, fluxsinglecolremain = F)
...
```{r alluvial5,dpi=600,fig.width=8,fig.height=2}
  
  get_alluvialplot(track,dcmembership,cols,fluxsinglecol = F,fluxbysource = T, fluxsinglecolremain = T)
...
```{r alluvial6,dpi=600,fig.width=7,fig.height=2}
  source("MTRfuncs.R",local = knitr::knit_global())  
  get_alluvialplot(track,dcmembership,cols,fluxsinglecol = F,fluxbysource = T, fluxsinglecolremain = T,
...
```{r alluvial7,dpi=600,fig.width=7,fig.height=2}
  get_alluvialplot(track,dcmembership,cols,fluxsinglecol = F,fluxbysource = T, fluxsinglecolremain = T,
                   rlabels=c("a","b","c","d","e","f"),
                   rmargins = c(0.2,0.2,0.8,0.8),
                   figwidth=7,figheight=2,
  ...
  #TODO: EXPOSE MORE ARGUMENTS BETWEEN R AND PYTHON - CLUSTER LABELS, CLUSTER GAPS ETC.
